...
```
工作中目前需要用到的和数据库有关的Java包是com.j256.ormlite.dao  

之前遇到了一个问题是如何从数据库中取出来所需要的几列数据，想了很多办法没有实现，最后只能利用class.get()方法把数据一列一列取出来再进行拼接  
代码十分丑陋又复杂
```java
public static List<String> convertToList(List<ValPinEntity> aList) throws IOException {
        List<String> result = new ArrayList<>();
        for (ValPinEntity a : aList) {
            String lines = String.format(
                    "%-25s%-25s%-40s%-5s", a.getPartRef(),a.getPinNumber(),a.getPinNetName(),a.getPinUnits());
            result.add(lines);
        }
        return result;
    }
```  
后来发现这个包中自带了方法可以类似SQL那样查询  
